...
val splitAmount: String = "",          // Сумма для новой операции
val splitCategory: CategoryUi? = null // категория для новой операции
```

**ViewModel методы:**
```kotlin
fun enableSplitMode()              // Вход в режим разделения
fun disableSplitMode()            // Выход из режима
fun updateSplitAmount(value: String) // Изменение суммы (валидация ≤ общей)
fun updateSplitCategory(category)  // Выбор категории
fun getRemainingAmount(): Double   // Общая сумма - splitAmount
```
